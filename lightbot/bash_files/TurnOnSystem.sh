...
# Bash file for controlling the System - Turning On the system
# For rotating table (placced on the XY platform), I am not using the XY platform movements.
...
# make sure the camera is properly stopped before restarting else it will give error
gnome-terminal --tab --title="Camera" -- bash -c "$SOURCE_CMD; sleep 5; roslaunch avt_vimba_camera mono_camera.launch; exec bash"
gnome-terminal --tab --title="EVA Moveit" -- bash -c "$SOURCE_CMD; sleep 10; roslaunch eva_without_rail_moveit_config EvaHardware.launch; exec bash"
gnome-terminal --tab --title="EVA Service" -- bash -c "$SOURCE_CMD; sleep 15; rosrun eva_control eva_service.py; exec bash"
gnome-terminal --tab --title="EVA Control" -- bash -c "$SOURCE_CMD; sleep 20; rosrun eva_control eva_ros_control_server.py; exec bash"
gnome-terminal --tab --title="Lightbot" -- bash -c "$SOURCE_CMD; sleep 25; roslaunch lightbot lightbot.launch; exec bash"
gnome-terminal --tab --title="LightBot Client" -- bash -c "sleep 30; python3 /home/esirem/Desktop/M2_Thesis/4-RTI_Bot/eva_rti_ws/src/lightbot/scripts/sample_client.py; bash"
