#! /usr/bin/env python3
"""
Before running or using this script, please go through the "main.py" script in the same folder.
This script controls the data acquistion of a large object, when the turtable is rotated manually in between the two acquisitions.
 - The goal is to acquire a larger object in smaller segments. 
...
class RobotControl():
    ...
    def execute_acquisition(self, req):
        ...
        def run_two_acquisitions():
            # First acquisition (FRONT)
            rospy.loginfo("Starting first acquisition (FRONT side)...")
            self.acquire()

            # Reset all direction markers to green
            self.direction_markers = utilities.reset_direction_marker_colors(self.direction_markers)
            self.direction_pub.publish(self.direction_markers)  # re-publishing updated markers

            rospy.loginfo("\n==== Manual Step Required ====\n"
                        "First acquisition complete. Please rotate the object.\n"
                        "Press ENTER in the terminal when done to continue with second acquisition...")
            input(">>> Press ENTER to continue !")

            # Reseting for second acquisition
            self.missed_poses = []
            self.counter = 0

            # Second acquisition (BACK)
            rospy.loginfo("Starting second acquisition (BACK side)...")
            self.acquire()

            rospy.loginfo("Both acquisitions completed successfully.")

        threading.Thread(target=run_two_acquisitions).start()
        return True
