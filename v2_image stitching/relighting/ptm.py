"""
This module contains the code for fitting and using a Polynomial Texture Map (PTM) model.
The PTM model is used for relighting images based on light positions in Cartesian coordinates.
It includes methods for computing the PTM basis, fitting the model to target images,
and relighting images using the fitted model.
"""
import numpy as np
    ...
    def relight(self, lps_cartesian, save_paths, target_images=None):
        ...
        for i, path in enumerate(save_paths):
            ...
            if target_images is not None:
                ...
                plt.subplot(3, 3, 5)
                # Convert both images to grayscale (even if they are effectively already)
                target_gray = cv2.cvtColor(target_img, cv2.COLOR_RGB2GRAY)
                relit_gray = cv2.cvtColor(relit_img, cv2.COLOR_RGB2GRAY)

                # Compute histograms
                target_hist = cv2.calcHist([target_gray], [0], None, [256], [0, 256])
                relit_hist = cv2.calcHist([relit_gray], [0], None, [256], [0, 256])

                # Plot histograms
                plt.plot(target_hist, color='green', linestyle='-', label='Target')
                plt.plot(relit_hist, color='blue', linestyle='--', label='Relit')
                ...
                # Add final label with light position
                plt.suptitle(f'Comparison (LP: x={lps_cartesian[i, 0]:.2f}, y={lps_cartesian[i, 1]:.2f}, z={lps_cartesian[i, 2]:.2f})',
                            fontsize=14)
