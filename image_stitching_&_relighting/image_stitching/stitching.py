...
def stitch_images(acqA_folder, acqB_folder, output_folder, matches_folder, overlap_folder, use_multiband=False):
    output_folder.mkdir(parents=True, exist_ok=True)
    sift = cv2.SIFT_create()
    bf = cv2.BFMatcher()

    # use when using aligned images for stitching without light correction:
    # resultsA_folder = acqA_folder
    # resultsB_folder = acqB_folder
    
    # use when using light corrected images for stitching:
    ...
    for filename in sorted(os.listdir(resultsA_folder)):
        ...
        if use_multiband:
            canvas = multiband_blending(imgA, imgB, H, levels=6, filename=filename, overlap_folder=overlap_folder)
        else:
            mask_B = (warped_B > 0)
            canvas = np.where(mask_B, warped_B, canvas)
            print(f"Using simple blending for {filename}")

        out_name = f"{'blend_' if use_multiband else ''}stitched_{filename}"
